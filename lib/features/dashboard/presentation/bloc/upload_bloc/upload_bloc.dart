...
class UploadBloc extends Bloc<UploadEvent, UploadState> {
  ...
  FutureOr<void> _onFilesUpload(FilesUploadEvent event, Emitter<UploadState> emit) async {
    ...
    if (state.files.isNotEmpty) {
      ...
      // await _firestoreService.uploadMultipleFiles(state.files);
      await Future.delayed(const Duration(seconds: 2));
      emit(state.copyWith(pageState: PageState.success, isButtonEnabled: true, files: []));
    } else {
      emit(state.copyWith(pageState: PageState.error, errorMessage: 'No files are selected', isButtonEnabled: true));
    }
  }
}
